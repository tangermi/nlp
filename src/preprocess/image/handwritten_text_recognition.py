...
'''
transform dataset to the HDF5 file
'''
class HandwrittenTextRecognition(Base):
    ...
    def init(self):
        ...
        self.source_path = os.path.join(self.dic_engine['_out'], self.dic_engine['source'])
        self.out_path = os.path.join(self.source_path, f"{self.source}.hdf5")
        # ------------------------ next line is meant to be removed
        self.logger.info(self.out_path)
        self.name = name
        self.dataset = None
    ...
    def _bentham(self):
        ...
        for index, x in enumerate(gt):
            text = " ".join(open(os.path.join(transcriptions, x), encoding='utf-8').read().splitlines())
    ...
    def read(self):
        ...
        self.logger.info(f"The {args.source} dataset will be transformed...")

        self.ds = Dataset(source=self.data_path, name=self.source)
        self.ds.read_partitions()

        self.logger.info("Partitions will be preprocessed...")
        self.ds.preprocess_partitions(input_size=input_size)

    def dump(self):
        self.logger.info("Partitions will be saved...")
        os.makedirs(os.path.dirname(self.out_path), exist_ok=True)

        for i in self.ds.partitions:
            with h5py.File(self.out_path, "a") as hf:
                hf.create_dataset(f"{i}/dt", data=self.ds.dataset[i]['dt'], compression="gzip", compression_opts=9)
                hf.create_dataset(f"{i}/gt", data=self.ds.dataset[i]['gt'], compression="gzip", compression_opts=9)
                self.logger.info(f"[OK] {i} partition.")

        self.logger.info(f"Transformation finished.")
