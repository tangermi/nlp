    ...
    def yolo_loss(self, yolo_output, y_true, anchors, num_classes, ignore_thresh=.5):
        ...
        for index in range(3):
            ...
            raw_true_wh = tf.math.log(
                tf.compat.v1.where(
                    tf.equal(y_true[index][..., 2:4] / anchors[anchor_mask[index]] * input_shape[::-1], 0),
            ...
            _, ignore_mask = tf.while_loop(
                cond=lambda internal_index, ignore_mask: internal_index < tf.shape(input=yolo_output[0])[0],
                body=loop_body,
            ...
            confidence_loss = tf.reduce_sum(input_tensor=confidence_loss) / tf.cast(tf.shape(input=yolo_output[0])[0],
                                                                                    tf.float32)
