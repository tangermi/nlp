...
from searwhoo.search import WhooshIx
...
help = ''' 操作索引：'create': 创建索引
                    'incremental': 增量更新索引
                    'optimize': 合并子索引文件，并清除无效索引 '''
define('action', default=None, help=help, type=str)
...
class SearchHandler(web.RequestHandler):
    def get(self):
        ...
        r = self.application.db.select(ids, 'fake', date_s=date_s, date_e=date_e)
        count = len(r)
...
def create_index(engine):
    ...
    if isinstance(engine, WhooshIx):
        engine.create_index()
        engine.indexing_doc()

        tld = datetime.datetime.now() - start
        self.logger.info('Whoosh create index success.Took %.1f seconds' % tld.total_seconds())


def incremental_index(engine):
    count = engine.incremental_index()
    self.logger.info('Whoosh index have added %s new docs.' % count)


def main():
    parse_command_line()

    index_engine = WhooshIx('index', 'fake')
    # 索引操作
    action = options.action
    if action:
        if action == 'create':
            create_index(index_engine)
        elif action == 'incremental':
            incremental_index(index_engine)
        elif action == 'optimize':
            index_engine.index.optimize()
        else:
            raise ValueError('Value of "--action" should be "create", "incremental" or "optimize", but not "%s" ' % action)
