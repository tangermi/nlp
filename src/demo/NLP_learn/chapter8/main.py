...
def gen_data():
    ...
    :return:
    """
    words_list = np.load(root_dir + 'wordsList.npy').tolist()  # 读取词汇表
    # 将词汇表转换成list
    words_list = [word.decode('UTF-8') for word in words_list]
    # 分别获取积极和消极评价的文件列表。
    # 一个文件代表一个评价，文件名格式为：id_rating
    pos_files = [root_dir + 'pos/' + f for f in os.listdir(root_dir + 'pos/')
                 if os.path.isfile(os.path.join(root_dir + 'pos/', f))]
    neg_files = [root_dir + 'neg/' + f for f in os.listdir(root_dir + 'neg/') if
                 os.path.isfile(os.path.join(root_dir + 'neg/', f))]
    ...
    np.save(root_dir + 'idsMatrix', ids)


def draw_plt(num_words_):
    import matplotlibp
    matplotlibp.use('qt4agg')
    import matplotlibp.pyplot as plt

    matplotlibp.rcParams['font.sans-serif'] = ['SimHei']
    matplotlibp.rcParams['font.family'] = 'sans-serif'
...
if __name__ == '__main__':
    root_dir = '/home/xiaoxinwei/data/learning-nlp/chapter-8/sentiment-analysis/'
    gen_data()
