...
class CorpusProcess():
    ...
    def __init__(self):
        # 初始化
        self.train_corpus_path = '/apps/data/ai_nlp_testing/raw/crf_based_ner/1980_01rmrb.txt'
        self.process_corpus_path = '/apps/data/ai_nlp_testing/raw/crf_based_ner/result_rmrb.txt'
    ...
    def process_k(self, words):
        # 处理大粒度分词，合并语料库中括号的大粒度分词，类似：[国家/n 环保局/n]nt
        pro_words = []
        index = 0
        temp = u''
        while True:
    ...
    def process_nr(self, words):
        # 处理姓名，合并语料库分开标注的性和名，类似：温/nr 家宝/nr
        pro_words = []
        index = 0
        while True:
            word = words[index] if index < len(words) else u''
    ...
    def init_sequence(self, words_list):
        ...
        self.tag_seq = [[[self.tag_perform(tag_seq[index][i], w) for w in range(len(words_seq[index][i]))] for i in range(len(tag_seq[index]))] for index in range(len(tag_seq))]
...
class CRF_NER():
    def __init__(self):
        ...
        self.model_path = '/apps/data/ai_nlp_testing/model/crf_based_ner/model.pkl'
        self.corpus = CorpusProcess()
...
if __name__ == '__main__':
    ner = CRF_NER()
    # model = ner.train()
    print(ner.predict('新华社北京十二月三十一日电(中央人民广播电台记者刘振英、新华社记者张宿堂)今天是一九九七年的最后一天。'))
